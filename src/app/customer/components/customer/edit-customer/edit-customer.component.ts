...
export class EditCustomerComponent implements OnInit {
  ...
  initFormGroup() {
    this.form = this.formBuilder.group({
      ...
      TableTel: new FormControl(this.customer.TableTel, [Validators.required, Validators.pattern(/^(\(\d{2,4}\)\s{0,1}\d{6,9})$|^\d{8,13}$|^\d{3,5}\s?\d{3}\s?\d{3,4}$|^[\d\(\)\s\-\/]{6,}$/)]),
      Tel: new FormControl(this.customer.Tel, [Validators.required, Validators.pattern(/^(\(\d{2,4}\)\s{0,1}\d{6,9})$|^\d{8,13}$|^\d{3,5}\s?\d{3}\s?\d{3,4}$|^[\d\(\)\s\-\/]{6,}$/)]),
      Fax: new FormControl(this.customer.Fax, [Validators.pattern(/^(\(\d{2,4}\)\s{0,1}\d{6,9})$|^(\+?\d{1,}(\s?|\-?)\d*(\s?|\-?)\(?\d{2,}\)?(\s?|\-?)\d{3,}\s?\d{3,})$/)]),
      ...
      TaxCode: new FormControl(this.customer.TaxCode, [Validators.pattern(/^[\d\(\)\s\-\/]*$/)]),
      ...
      DeputyUM: new FormGroup({
        ...
        PhoneNumber: new FormControl(this.customer.DeputyVM.PhoneNumber, [Validators.pattern(/^(\(\d{2,4}\)\s{0,1}\d{6,9})$|^\d{8,13}$|^\d{3,5}\s?\d{3}\s?\d{3,4}$|^[\d\(\)\s\-\/]{6,}$/)]),
        Email: new FormControl(this.customer.DeputyVM.Email, [Validators.email]),
      }),
...
export class DuplicatedValidation {
  static isDuplicated(mainComponent: EditCustomerComponent, AC: AbstractControl) {
    ['TableTel', 'Tel', 'Email', 'TaxCode', 'Name'].forEach((field) => {
