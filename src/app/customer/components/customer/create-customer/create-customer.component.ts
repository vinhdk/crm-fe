    ...
    this.form = this.formBuilder.group(
      {
        ...
        Tel: new FormControl('', [
          Validators.pattern(
            /^(\(\d{2,4}\)\s{0,1}\d{6,9})$|^\d{8,13}$|^\d{3,5}\s?\d{3}\s?\d{3,4}$|^[\d\(\)\s\-\/]{6,}$/
          ),
        ]),
        Fax: new FormControl('', [
          Validators.pattern(
            /^(\(\d{2,4}\)\s{0,1}\d{6,9})$|^(\+?\d{1,}(\s?|\-?)\d*(\s?|\-?)\(?\d{2,}\)?(\s?|\-?)\d{3,}\s?\d{3,})$/
          ),
        ]),
        ...
        TaxCode: new FormControl('', [Validators.pattern(/^[0-9]*$/)]),
    ...
    if (this.customerService.customerList) {
      result =
        this.customerService.customerList.findIndex(
          (e) =>
            e[field] && value && e[field].toLowerCase() === value.toLowerCase()
        ) >= 0;
    }
