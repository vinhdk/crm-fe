...
export class NotificationCenterComponent implements OnInit {
  ...
  nextPage() {
    ...
    if (this.hasMore) {
      ...
      this.currentPage++;
      this.hasMore = this.pageSize * this.currentPage < this.notifications.length;
    }
  }
      ...
      .then(
        (response: Notification[]) => {
          ...
          if (this.currentPage === 0) {
            ...
            this.currentPage = 0;
            this._notifications = [];
            for (let i = 0; i < n; i++) { this.nextPage(); }
          }
        },
  ...
  routingNoti(index: number, notification: Notification) {
    ...
    if (notification.NData) {
      try {
        ...
        if (noti_NData.hasOwnProperty('type') && noti_NData.type === 0 && noti_NData.url) {
          if (noti_NData.url.includes('.xlsx')) {
            window.open('http://crmbe.hisoft.vn/api/Customers/File?filePath=' + noti_NData.url, '_blank');
          } else {
  ...
  setUnreadCount() {
    this.unreadCount = this.notifications.filter(e => e.IsSeen === false).length;
  }
